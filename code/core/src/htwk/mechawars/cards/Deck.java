...
public class Deck {
    ...
    public void initDeck() {
        ...
        for (int i = 0; i < 6; i += 1) {
            deck.add(new Card(Type.turn, (byte) 2, priority));
            priority += 10;
        }

        // 18 Cards: Left-Turn & Right-Turn / priority 70-420
        for (int i = 0; i < 18; i += 1) {
            deck.add(new Card(Type.turn, (byte) 3, priority));
            priority += 10;
            deck.add(new Card(Type.turn, (byte) 1, priority));
            priority += 10;
        }

        // 6 Cards: Backup / priority 430-480
        for (int i = 0; i < 6; i += 1) {
            deck.add(new Card(Type.mov, (byte) -1, priority));
            priority += 10;
        }
        ...
        for (int i = 0; i < 12; i += 1) {
            deck.add(new Card(Type.mov, (byte) 2, priority));
            priority += 10;
        }

        // 6 Cards: Move 3 / priority 790-840
        for (int i = 0; i < 6; i += 1) {
            deck.add(new Card(Type.mov, (byte) 3, priority));
            priority += 10;
        }
    }
